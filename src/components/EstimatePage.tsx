...
import {
  ...
  resolvedGrade, pricingGrade, resolvedQty, resolvedTotal, shouldShowGradeChip,
...
interface Inputs {
  ...
  fittingTemplates?: AppliedTemplate[]; // fixture-linked fitting templates (Phase 5)
...
// pipes, fittings) is user-added by nature, so it is unaffected.
...
// ─── FIXTURE-TEMPLATE UI (Phase 5) ────────────────────────────────────────────
// Per-row Product control. Runs the row's product_filter against plumblink_materials
// (Phase 3) to populate the dropdown, or falls back to free-text for Custom rows and
// the Manual/custom line sentinel. Seeds a Suggested row's default price on load
...
export default function EstimatePage() {
  ...
  // Fixture-template management (Phase 5). Applying fetches the template's rows
  // and builds live TemplateRowInstances; the basis input scales every row's
  // pricing (fixture count, or pipe run metres for system templates).
