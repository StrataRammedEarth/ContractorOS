...
export function printInvoiceDocument(args: {
  ...
  sellExVat: number;
  // Optional contractor identity / config (from settings). Default to previous
  // hardcoded behaviour when omitted.
  vatRate?: number;
  business?: string;
  vatNumber?: string;
  terms?: string;
  ...
  const business = (args.business ?? "").trim() || "[Your Plumbing Business]";
  const fromDetails = args.vatNumber ? `VAT no. ${args.vatNumber}` : "VAT vendor details / contact details";
...
<div class="parties"><div class="party"><div class="party-label">Invoice To</div><h3>${esc(inputs.clientName || 'Client')}</h3><p>${esc(inputs.projectName || 'Project / address not specified')}</p></div><div class="party"><div class="party-label">From</div><h3>${esc(business)}</h3><p>${esc(fromDetails)}</p></div></div>
...
<div class="totals"><div class="total-row"><span>Materials used excl VAT</span><span>R ${fmtN(materialTotal)}</span></div><div class="total-row"><span>Labour excl VAT</span><span>R ${fmtN(labourTotal)}</span></div><div class="total-row"><span>Commercial allowances and margin excl VAT</span><span>R ${fmtN(allowance)}</span></div><div class="total-row"><span>Subtotal excl VAT</span><span>R ${fmtN(sellExVat)}</span></div><div class="total-row"><span>VAT @ ${vatPct}%</span><span>R ${fmtN(vat)}</span></div></div>
<div class="total-final"><span>Total amount due</span><span>R ${fmtN(totalDue)}</span></div>
<div class="terms"><div class="terms-box"><h4>Banking Details</h4><div>${esc(bankingDetails)}</div></div><div class="terms-box"><h4>Invoice Terms</h4><div>— Due date: ${displayDate(dueDate)}</div><div>— Invoice records actual materials and labour used.</div><div>— Line confidence grades are retained for audit and do not block invoice issue.</div>${args.terms ? `<div>— ${esc(args.terms)}</div>` : ''}</div></div>
<div class="footer">ContractorOS — Invoice · ${esc(invoiceRef)} · Total amount due includes ${vatPct}% VAT</div>
</div></body></html>`;
