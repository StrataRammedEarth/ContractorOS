...
function fail(label) { throw new Error(`Missing patch anchor: ${label}`); }
function replaceOnce(needle, value, label) {
  if (!s.includes(needle)) fail(label);
  s = s.replace(needle, value);
  changed = true;
}
function insertBeforeIndex(index, value) {
  s = s.slice(0, index) + value + s.slice(index);
  changed = true;
}
function indexAfter(needle, from, label) {
  const idx = s.indexOf(needle, from);
  if (idx < 0) fail(label);
  return idx;
}

const hasInvoice = s.includes('documentType, setDocumentType');
const hasFittings = s.includes('interface FittingLine');

if (!s.includes('printInvoiceDocument') && !s.includes('plumblink-fittings')) {
  replaceOnce(
...
} else if (!s.includes('plumblink-fittings')) {
  replaceOnce(
    'import { addDays, DEFAULT_BANKING_DETAILS, isoDate, printInvoiceDocument, type DocumentType, type InvoiceMeta } from "@/lib/invoice-document";\n',
    'import { addDays, DEFAULT_BANKING_DETAILS, isoDate, printInvoiceDocument, type DocumentType, type InvoiceMeta } from "@/lib/invoice-document";\nimport { COMPRESSION_FITTINGS, FITTING_SIZE_GROUPS, fittingsForSizeGroup } from "@/lib/plumblink-fittings";\n',
    'fittings import',
...
`<div style={{color:C.muted,fontSize:11,letterSpacing:1,textTransform:"uppercase"}}>{documentType === "invoice" ? "Invoice Total (excl. VAT)" : "Sell Price (excl. VAT)"}</div>`, 'EstimateTab amount label');

  replaceOnce(
`<div style={{color:C.muted,fontSize:10,marginTop:6}}>{issuable?"✓ Internal use OK":"⚠ Not client-issuable"}</div>`,
`<div style={{color:C.muted,fontSize:10,marginTop:6}}>{documentType === "invoice" ? "✓ Invoice issuable — actuals recorded" : issuable ? "✓ Quote client-issuable" : "⚠ Quote not client-issuable"}</div>`, 'EstimateTab gate label');
...
`<button onClick={onPrintQuote} disabled={!issuable} style={{marginTop:10,padding:"7px 16px",borderRadius:6,border:"none",background:issuable?C.gold:C.muted,color:C.navy,cursor:issuable?"pointer":"not-allowed",fontWeight:700,fontSize:12}}>{documentType === "invoice" ? "⬇ Download Invoice" : "⬇ Download Quote"}</button>`, 'EstimateTab button');
...
`{documentType === "invoice" ? "✓ INVOICE ISSUABLE — actuals pass with audit grades retained" : issuable ? "✓ QUOTE CLIENT-ISSUABLE" : "✗ QUOTE NOT CLIENT-ISSUABLE — resolve flagged inputs first"}`, 'LearnTab copy');

  replaceOnce(
`<SectionHeader>Assembly Flags — must clear before client issue</SectionHeader>`,
`<SectionHeader>{documentType === "invoice" ? "Assembly Flags — retained for invoice audit" : "Assembly Flags — must clear before client issue"}</SectionHeader>`, 'LearnTab flags heading');
...
`, 'output tab props');
  ...
  replaceOnce(`<div style={{color:C.slateL,fontSize:10}}>{quoteRef}</div>`, `<div style={{color:C.slateL,fontSize:10}}>{docRef}</div>`, 'header ref');
  replaceOnce(`<div style={{color:C.muted,fontSize:10,letterSpacing:0.5}}>SELL PRICE excl. VAT</div>`, `<div style={{color:C.muted,fontSize:10,letterSpacing:0.5}}>{documentType === "invoice" ? "INVOICE TOTAL excl. VAT" : "SELL PRICE excl. VAT"}</div>`, 'header amount label');
...
if (!hasFittings) {
  replaceOnce('interface GeyserMeta {\n', [
  ...
  replaceOnce('  const drainLines  = inp.drainLines ?? [];\n  const supplyMetres = sumMetres(supplyLines);\n', '  const drainLines  = inp.drainLines ?? [];\n  const fittingLines = inp.fittingLines ?? [];\n  const supplyMetres = sumMetres(supplyLines);\n', 'buildScope fitting local');

  const buildScopeStart = indexAfter('function buildScope', 0, 'buildScope start');
  const buildLabourStart = indexAfter('\n}\n\nfunction buildLabour', buildScopeStart, 'buildScope end');
  insertBeforeIndex(buildLabourStart, [
    '',
  ...
  const scopeStart = indexAfter('function ScopeModal', 0, 'ScopeModal start');
  const firstFilter = indexAfter('].filter(Boolean);', scopeStart, 'ScopeModal first filter');
  const secondFilter = indexAfter('].filter(Boolean);', firstFilter + 1, 'ScopeModal second filter');
  insertBeforeIndex(secondFilter, [
    '        ...((inputs.fittingLines ?? []).filter(l=>l.quantity>0)',
    '          .map(l=>`${l.quantity}× ${l.description} (${l.sizeGroup})`)),',
  ].join('\n') + '\n');

  replaceOnce('  drainLines:[pipeLineFrom("drainage","PVC",110,15)],\n};\n', '  drainLines:[pipeLineFrom("drainage","PVC",110,15)],\n  fittingLines:[],\n};\n', 'DEFAULT fittingLines');
...
'  const onScanDone',
  ...
  replaceOnce('      drainLines:  data.drainMetres>0  ? [pipeLineFrom("drainage","PVC",110,data.drainMetres)] : [],\n', '      drainLines:  data.drainMetres>0  ? [pipeLineFrom("drainage","PVC",110,data.drainMetres)] : [],\n      fittingLines: [],\n', 'scan fitting default');

  const fixtureStart = indexAfter('<SectionHeader>Fixtures — one line per product', 0, 'fixtures UI start');
  const fragmentEnd = indexAfter('        </>)}', fixtureStart, 'fixtures fragment end');
  insertBeforeIndex(fragmentEnd, [
...
console.log(changed ? 'ContractorOS patches applied.' : 'ContractorOS patches already applied.');
